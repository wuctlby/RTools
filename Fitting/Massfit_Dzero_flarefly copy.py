...
def Mass_fit_ff(inputFile, histoName, MassMin, MassMax, Rebin, SgnFunc, BkgFunc, FixSigma=False, FixSigmaFromFile='', iPt=0):
    ...
    # bkgMass = DataHandler(data=inputFile, histoname='hist_inv_mass_bkg_ptSmear_Prompt',
    #                         limits=[MassMin, MassMax], rebin=Rebin)

    refMass = DataHandler(data=inputFile, histoname='hist_inv_mass_ref_ptSmear1.5+phi_FD',
                          limits=[MassMin, MassMax], rebin=2)
    
    rovers = refMass.get_norm() / dataMassRebined.get_norm()
    ...
    if FixSigma:
        ...
        fitter.set_signal_initpar(0, "sigma", 0.01, limits=[0.001, 0.5])

    # Initiate fit parameters
    fitter.set_particle_mass(0, pdg_id=421, limits=[1.800, 1.900])
    fitter.set_signal_initpar(0, "frac", 0.01, limits=[0.0, 1.0])
    # fitter.set_background_initpar(0, "c0", 2)
    # fitter.set_background_initpar(0, "c1", -0.1)
    # fitter.set_background_initpar(0, "c2", -0.01)
    fitter.set_reflection_template(0, refMass, r_over_s=rovers)
    # fitter.set_background_template(0, bkgMass)
    # print(fitter.get_name_refl_pdf())
    # exit()

    # Fit
    fitter.mass_zfit()
    
    return fitter, rovers
...
def main(config_fit, inputFiles, outputDir, cent, suffix):
    ...
    hMasses, hSigmas, hRawYields, hBkgs, hSignificances, hChi2s, hSoverBs, hRoverS = [], [], [], [], [], [], [], []
    # hPuritys = []
    ...
    hRoverS = ROOT.TH1F(f"hRawYieldsRoverS", f"Reflection over Signal vs pT; pT (GeV/c); R / S", nPtBins, np.array(ptBins, dtype="float64"))
    hN = ROOT.TH1F(f"hParamN", f"Signal parameter N vs pT; pT (GeV/c); N", nPtBins, np.array(ptBins, dtype="float64"))
    halpha = ROOT.TH1F(f"hParamAlpha", f"Signal parameter Alpha vs pT; pT (GeV/c); Alpha", nPtBins, np.array(ptBins, dtype="float64"))
